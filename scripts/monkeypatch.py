...
def pack_data_points_naive(
    data_points: List[Dict], tokenizer: Any, model_max_length: int
) -> Dict:
    """Pack data points WITHOUT Flash Attention — standard 1D attention mask.

    Uses simple concatenation with position_ids reset per sequence.
    Cross-attention between packed sequences is possible (causal mask
    doesn't prevent it), but empirically this has minimal impact on
    training quality while giving ~2-3x throughput over padding.
    """
    input_ids = []
    label_ids = []
    position_ids = []
    ...
    pad_leng = model_max_length - len(input_ids)

    if tokenizer.padding_side == "right":
        input_ids = input_ids + [tokenizer.pad_token_id] * pad_leng
        label_ids = label_ids + [-100] * pad_leng
        attention_mask = [1] * (model_max_length - pad_leng) + [0] * pad_leng
        position_ids = position_ids + [0] * pad_leng
    else:
        input_ids = [tokenizer.pad_token_id] * pad_leng + input_ids
        label_ids = [-100] * pad_leng + label_ids
        attention_mask = [0] * pad_leng + [1] * (model_max_length - pad_leng)
        position_ids = [0] * pad_leng + position_ids
...
class PackedDataset(Dataset):
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        pack_fn = pack_data_points_FA if self.use_fa else pack_data_points_naive
        return pack_fn(group_data_points, self.tokenizer, self.max_input_length)
